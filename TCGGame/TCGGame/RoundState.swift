	...
	func nextPhase(action: RoundAction) -> RoundPhase {
		...
		case .Finish: // MARK: Action .Finish
			...
			if action.performedByPlayer1 {
				...
				if !kDevPretendThatPawnsAreAlwaysPlacedCorrectly && (nextState.posPawn1.x != level.goalConfigurationPawn1.x || nextState.posPawn1.y != level.goalConfigurationPawn1.y || !level.pawnPlayer1.rotationsMatch(nextState.rotationPawn1, rotation2: level.goalConfigurationPawn1.rotation)) {
				...
				if !kDevPretendThatPawnsAreAlwaysPlacedCorrectly && (nextState.posPawn2.x != level.goalConfigurationPawn2.x || nextState.posPawn2.y != level.goalConfigurationPawn2.y || !level.pawnPlayer2.rotationsMatch(nextState.rotationPawn2, rotation2: level.goalConfigurationPawn2.rotation)) {
